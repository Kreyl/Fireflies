...
static const uint32_t IData[(FLASH_PAGE_SIZE/4)] = { 0xCA115EA1 };

#if FLASH_2_BANKS
__attribute__ ((section("MyFlash2")))
static const uint32_t IData2[(FLASH_PAGE_SIZE/4)] = { 0xCA115EA1 };
#endif
...
uint8_t Save(uint32_t *ptr, uint32_t ByteSz) {
    return ProgramBuf(ptr, ByteSz, (uint32_t)&IData[0]);
}
